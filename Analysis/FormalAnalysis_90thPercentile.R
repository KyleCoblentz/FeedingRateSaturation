################################################################################
### Formal Analysis -- 90th percentile
################################################################################
...
### 90th percentile of predicted prey density
################################################################################

fit <- brm(frac_90 ~ Prey_Major_Group_1 + Pred_Major_Grouping_1 + Dim_factor + Habitat_Aquatic + log(Arena_size) +
...
residuals_fit <- residuals_fit %>% mutate(partial_predmass = Estimate + expit(0.062*log(forage_arena$PredMass_g)),
                                          partial_preymass = Estimate + expit(-0.30*log(forage_arena$Mass_g)),
                                          partial_temp = Estimate + expit(0.063*forage_arena$Temp_C + (-0.0019*forage_arena$Temp_C^2)),
                                          partial_arena = Estimate + expit(0.05*log(forage_arena$Arena_size)),
...
Pred_Mass_PreyTaxa <- ggplot(data = residuals_fit, aes(x = log(predmass), y = partial_predmass, color = prey_major_taxa)) + geom_point() + 
  theme_cowplot() + scale_colour_brewer(palette = 'Paired', name = 'Prey Taxa') + geom_function(fun = function(x) expit(0.062*x), color = 'black') +
...
Pred_Mass_PredTaxa <- ggplot(data = residuals_fit, aes(x = log(predmass), y = partial_predmass, color = pred_major_taxa)) + geom_point() + 
  theme_cowplot() + scale_colour_brewer(palette = 'Paired', name = 'Predator Taxa') + geom_function(fun = function(x) expit(0.062*x), color = 'black') + 
  xlab('log Predator Mass (g)') + ylab('Partial Residual of Saturation Index') + theme(axis.title.y = element_blank())


Prey_Mass_PreyTaxa <- ggplot(data = residuals_fit, aes(x = log(preymass), y = partial_preymass, color = prey_major_taxa)) + geom_point() + 
  theme_cowplot() + scale_colour_brewer(palette = 'Paired', name = 'Prey Taxa') + geom_function(fun = function(x) expit(-0.3*x), color = 'black') +
...
Prey_Mass_PredTaxa <- ggplot(data = residuals_fit, aes(x = log(preymass), y = partial_preymass, color = pred_major_taxa)) + geom_point() + 
  theme_cowplot() + scale_colour_brewer(palette = 'Paired', name = 'Predator Taxa') + geom_function(fun = function(x) expit(-0.3*x), color = 'black') +
  xlab('log Prey Mass (g)') + ylab('') + theme(legend.position = 'none')+ theme(axis.title.y = element_blank())

Temp_PreyTaxa <- ggplot(data = residuals_fit, aes(x = temp, y = partial_temp, color = prey_major_taxa)) + geom_point() + 
  theme_cowplot() + scale_colour_brewer(palette = 'Paired', name = 'Prey Taxa') + geom_function(fun = function(x) expit(0.063*x - 0.0019*x^2), color = 'black') +
...
Temp_PredTaxa <- ggplot(data = residuals_fit, aes(x = temp, y = partial_temp, color = pred_major_taxa)) + geom_point() + 
  theme_cowplot() + scale_colour_brewer(palette = 'Paired', name = 'Predator Taxa') + geom_function(fun = function(x) expit(0.063*x - 0.0019*x^2), color = 'black')  + 
  xlab('Temperature (C)') + ylab('') + theme(legend.position = 'none') + theme(axis.title.y = element_blank())

Arena_PreyTaxa <- ggplot(data = residuals_fit, aes(x = log(arena_size), y = partial_arena, color = prey_major_taxa)) + geom_point() + 
  theme_cowplot() + scale_colour_brewer(palette = 'Paired', name = 'Prey Taxa') + geom_function(fun = function(x) expit(0.05*x), color = 'black') +
  xlab('log Arena Size') + ylab('') + theme(legend.position = 'none') + theme(axis.title.y = element_blank())
...
fit_prey_effect <- data.frame(Taxa = as.factor(c('Intercept', levels(forage_arena$Prey_Major_Group_1)[-1])),
                              Effect = fixef(fit)[1:9,1],
                              Upper = fixef(fit, probs = c(0.05, 0.95))[1:9,4],
                              Lower = fixef(fit, probs = c(0.05, 0.95))[1:9,3])
...
save_plot(filename = 'CovariatesPlot_90thPercentile.png', plot = TogetherPlots, ncol =2.4, nrow = 2.4)
